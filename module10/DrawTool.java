...
class DrawObject {
    ...
    ArrayList<Double> curveX, curveY;   // For curves
}

public class DrawTool extends JPanel {

    // What we draw: points, lines, ovals and rectangles.
    static java.util.List<DrawObject> points, lines, ovals, rectangles, images, labels, scribbles, eqnLines, curves;
    ...
    static Color curveColor = Color.blue;
    ...
    static
    {
        ...
        curves = Collections.synchronizedList (new ArrayList<DrawObject>());
    ...
    public static void drawCurve (ArrayList<Double> xValues, ArrayList<Double> yValues)
    {
	if ( (xValues == null) || (yValues == null) || (xValues.size() != yValues.size()) || (xValues.size() <= 1) ) {
	    return;
	}
	DrawObject R = new DrawObject ();
	R.color = curveColor;
	R.curveX = xValues;   R.curveY = yValues;
	R.sequenceNum = currentSequenceNum;
	R.drawStroke = drawStroke;
	synchronized(curves) { curves.add (R); }
	drawArea.repaint ();
    }

    public static void drawCurve (double[] x, double[] y)
    {
	if ((x==null) || (y==null)) {
	    return;
	}
    ...
    public static void setCurveColor (String colorString)
    {
	curveColor = toColor (colorString, rectangleColor);
    }
	...
	// Draw the objects.
	drawObjects (g, points, lines, ovals, rectangles, images, labels, eqnLines, curves);
	if (animationMode) {
	    drawObjects (g, animPoints, animLines, animOvals, animRectangles, null, labels, eqnLines, curves);
    ...
    void drawObjects (Graphics g, java.util.List<DrawObject> points, java.util.List<DrawObject> lines, java.util.List<DrawObject> ovals, java.util.List<DrawObject> rectangles, java.util.List<DrawObject> images, java.util.List<DrawObject> labels, java.util.List<DrawObject> eqnLines, java.util.List<DrawObject> curves)
    {
	try {
	    // Curves
	    synchronized(curves) {	 
    ...
    void drawCurve (Graphics g, DrawObject C)
    {
	if (C == null) {
	    return;
	}
	if ((sequencingOn) && (C.sequenceNum!=currentSequenceNumDisplay)) {
	    return;
	}
	...
	g.setColor (C.color);
	Graphics2D g2 = (Graphics2D) g;
	g2.setStroke (C.drawStroke);
	for (int i=0; i<C.curveX.size()-1; i++) {
	    // Draw a line from i-th point to i+1-st point.
	    double x1d = C.curveX.get(i);
	    double y1d = C.curveY.get(i);
	    double x2d = C.curveX.get(i+1);
	    double y2d = C.curveY.get(i+1);
	    int x1 = (int) ( (x1d-minX)/(maxX-minX) * (D.width-2*inset) );
	    int y1 = (int) ( (y1d-minY)/(maxY-minY) * (D.height-2.0*inset) );
	    int x2 = (int) ( (x2d-minX)/(maxX-minX) * (D.width-2*inset) );
	    int y2 = (int) ( (y2d-minY)/(maxY-minY) * (D.height-2.0*inset) );
	    g.drawLine (inset+x1, D.height-y1-inset, inset+x2, D.height-y2-inset);
	}
    }
